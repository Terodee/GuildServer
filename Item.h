...
#define DEF_EQUIPPOS_NONE		0		// ÀåÂøÀÌ ºÒ°¡´ÉÇÏ´Ù.
...
//2006-3-4 added by jcfly
#define DEF_EQUIPPOS_FAIRY		14
...
#define DEF_ITEMTYPE_USE_DELETE 3       
...
#define DEF_ITEMTYPE_USE_DELETE_DEST			11
...
public:
	...
	int		iComposeItemInfo(char *pData);
	int		iDecodeItemInfo(char* pData);
	//2006-3-29 added by jcfly. Between client and server
	int		iComposeClientItemInfo(char *pData);
	int		iDecodeClientItemInfo(char *pData);
	
	//2006-4-10 added by jcfly.guild item level
	char	cGetLevel(){
		return (char)((m_dwAttribute & 0x0E) >>1);
	}
	void	SetLevel(char cLevel)	{
		m_dwAttribute &= 0xFFFFFFF1;
		m_dwAttribute |= (cLevel << 1);
	}
